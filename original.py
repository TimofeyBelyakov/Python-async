...
while True:
    ...
    while True:
        ...
        if request is None:
            ...
            # Поскольку буфер отправки пустой, то мы получаем ответ сразу.
            # Как только буфер наполнится, то .send() станет блокировать выполнение дальнейшего кода. Мы будем
            # вынуждены ждать, пока кто-нибудь не прочитает данные из буфера и он не очистится, чтобы сокет смог туда
            # что-то записать.
