...
# В Django - список URL_PATTERNS, где каждый элемент - это объект url или path. Эти объекты принимают первым аргументом
# url, а вторым - функцию view, которая обрабатывает запрос по этой url. Пример: path("/index", index).
URLS = {
    "/": index,
    "/blog": blog
}
...
def run():
    ...
    # Поскольку мы не знаем, сколько будет длиться соединение (взаимодействие) с клиентом, то используется бесконечный
    # цикл.
    while True:
        # Сработает, когда сервер примет подключение от клиента. Возвращает сокет и адрес клиента.
        client_socket, addr = server_socket.accept()

        # Сработает, когда во входящем буфере появятся данные от клиента, то есть запрос.
        # Принимает кол-во байт в пакете, возвращает запрос в bytes.
