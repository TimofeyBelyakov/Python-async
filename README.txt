...
В предыдущих задачах мы объявляли список событий (задач), который мы определённым образом обрабатывали в событийном
цикле. Такими событиями были сокеты, которые изменяли своё состояние. Каждый сокет был ассоциирован с какой-то
функцией-обработчиком, и когда он менял своё состояние, то вызывалась эта функция. Такая ассоциация была реализована
с помощью namedtuple (2_selectors.py) или с помощью словарей (3_async_gens.py), которые прокручивались в событийном
цикле.
