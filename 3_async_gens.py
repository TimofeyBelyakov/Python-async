...
tasks = []
# Словари, в ключах которых сокеты, а в значениях - генераторы.
to_read = {}
to_write = {}
...
def event_loop():
    ...
    # Пока хотя бы один из объектов не пустой.
    while any([tasks, to_read, to_write]):

        # Данный цикл добавляет в tasks генераторы.
        while not tasks:
            # В функцию select() можно передавать любые итерируемые объекты, которые при проходе в цикле давали бы
            # файловые объекты. Словари при проходе в цикле дают ключи, а здесь в качестве ключей - сокеты.
