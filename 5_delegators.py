...
def delegator(gen):
    ...
    # В других языках конструкция yield from называется await. Смысл его заключается в том, что вызывающий код напрямую
    # управляет работой подгенератора. Пока это происходит, делегатор остаётся заблокированным: он вынужден ожидать,
    # когда подгенератор закончит свою работу. Поэтому подгенератор должен содержать в себе механизм, завершающий его
    # работу.
